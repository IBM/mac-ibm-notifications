...
public final class ConfigurableParameters {
    static let set = ["default_popup_bar_title": "NADefaultPopupBarTitle",
                      ...
                      "default_banner_timeout": "NADefaultBannerTimeout",
    ...
    /// The default timeout for the banner in seconds.
    /// By default no timeout is set.
    static var defaultBannerTimeout: Int? {
        if let timeoutString = UserDefaults.standard.string(forKey: "NADefaultBannerTimeout") {
            return Int(timeoutString)
        }
        return nil
    }
