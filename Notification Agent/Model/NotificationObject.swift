...
//
//  swiftlint:disable line_length trailing_whitespace compiler_protocol_init
...
public final class NotificationObject: NSObject, Codable, NSSecureCoding {
    ...
    init(from dictionary: [String: Any]) throws {
        ...
        guard let typeRawValue = dictionary["type"] as? String,
            let type = UIType(rawValue: typeRawValue) else {
                throw NAError.dataFormat(type: .noTypeDefined)
        }
        self.type = type
        self.barTitle = dictionary["bar_title"] as? String
        self.title = dictionary["title"] as? String
        self.subtitle = dictionary["subtitle"] as? String
        self.iconPath = dictionary["icon_path"] as? String

        if let accessoryviewTyperawValue = dictionary["accessory_view_type"] as? String,
            let accessoryViewType = NotificationAccessoryElement.ViewType.init(rawValue: accessoryviewTyperawValue),
            let accessoryViewPayload = dictionary["accessory_view_payload"] as? String {
        ...
        let mainButtonLabel = dictionary["main_button_label"] as? String ?? ConfigurableParameters.defaultMainButtonLabel
        let mainButtonCTAType = NotificationButton.CTAType(rawValue: dictionary["main_button_cta_type"] as? String ?? "none") ?? .none
        let mainButtonCTAPayload = (dictionary["main_button_cta_payload"] as? String) ?? ""
        ...
        if let secondaryButtonLabel = dictionary["secondary_button_label"] as? String,
            let secondaryButtonCTAType = NotificationButton.CTAType(rawValue: dictionary["secondary_button_cta_type"] as? String ?? "none") {
                self.secondaryButton = NotificationButton(with: secondaryButtonLabel, callToActionType: secondaryButtonCTAType, callToActionPayload: dictionary["secondary_button_cta_payload"] as? String ?? "")
        }

        if let tertiaryButtonLabel = dictionary["tertiary_button_label"] as? String,
            let tertiaryButtonCTATypeRawValue = dictionary["tertiary_button_cta_type"] as? String,
            let tertiaryButtonCTAType = NotificationButton.CTAType(rawValue: tertiaryButtonCTATypeRawValue),
            let tertiaryButtonCTAPayload = dictionary["tertiary_button_cta_payload"] as? String {
        ...
        if let helpButtonTypeRawValue = dictionary["help_button_cta_type"] as? String,
            let helpButtonCTAType = NotificationButton.CTAType(rawValue: helpButtonTypeRawValue),
            let helpButtonCTAPayload = dictionary["help_button_cta_payload"] as? String {
        ...
        self.timeout = dictionary["timeout"] as? String
        if let aotRaw = dictionary["always_on_top"] as? String {
            self.alwaysOnTop = aotRaw == "true"
        } else {
